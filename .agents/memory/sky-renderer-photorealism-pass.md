...
- Four new realism features added (2025-07): (1) `drawVenusWithPhase` — Venus crescent/gibbous using saveLayer+clipPath+terminator-ellipse path; (2) `drawTwilightAtmosphericBands` — pink/purple/blue layered gradient bands for civil/nautical/astronomical twilight; (3) star spectral redshift near horizon (alt<15°, Rayleigh absorption in drawStars); (4) golden hour city warm glow (PorterDuff.SCREEN RadialGradient on fgPath in drawCitySilhouette, peak at sunAlt=5°).
